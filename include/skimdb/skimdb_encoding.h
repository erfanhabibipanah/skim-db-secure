...
namespace skim {
    ...
    constexpr uint16_t UNprev_compressed_FLAG = 0x8000;  // 1 in MSB
    ...
    inline BlockType getBlockType(uint16_t block) {
        if (block & UNprev_compressed_FLAG) { return BlockType::Uncompressed; }
    ...
    public:
                    ...
                    while (inserted < 15) {
                        ...
                        if (inserted + next_count < 16) {
                            for (std::size_t bit = 0; bit < next_count; ++bit) {
                                literal |= (type == BlockType::OneRun) ? (1u << (14 - (inserted + bit))) : 0;
                            }
                            ...
                            blocks_[j] = static_cast<uint16_t>((type == BlockType::OneRun ? RUN_OF_ONES_FLAG : RUN_OF_ZEROS_FLAG) | remaining);
                            inserted += can_take;
        ...
        void serialize(Archive& archive) {
            archive(blocks_, next_seq_, prev_compressed_);
        } // serialize
