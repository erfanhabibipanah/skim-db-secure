...
namespace skim {
    ...
    public:
        static skim_db build(const std::size_t k,
                const std::size_t s,
                const std::size_t t,
                const fs::path& file_to_labels,
                const fs::path& dir) {

            auto [file_names, labels] = load_file_to_labels(file_to_labels);
        
            std::vector<roaring::Roaring> bitmaps(file_names.size());
            std::vector<size_t> idxs(bitmaps.size());
            std::iota(idxs.begin(), idxs.end(), 0);

            std::for_each(std::execution::par, idxs.begin(), idxs.end(),
                [&k, &s, &t, &dir, &file_names, &bitmaps](size_t i) {
