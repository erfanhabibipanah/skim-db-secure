#ifndef SKIMDB_SIPER_RLWE_H
#define SKIMDB_SIPER_RLWE_H
...
public:
    ...
    [[nodiscard]] auto extract_lwe_key() const -> siper_matrix<std::uint64_t> {
        ...
        siper_matrix<std::uint64_t> lwe_s{poly_degree, ctx_.log_q()};
...
inline auto ct_extract_lwe(
    ...
    std::uint64_t num_samples) -> siper_matrix<std::uint64_t>
{
    ...
    siper_matrix<std::uint64_t> lwe_ct{num_samples, ctx.log_q()};
...
inline auto compute_hint_ntt(
    ...
    std::uint64_t sqrt_N) -> siper_matrix<std::uint64_t>
{
    ...
    siper_matrix<std::uint64_t> hint{sqrt_N, poly_degree, log_q};

    auto rns_size = poly_degree * num_moduli;

    auto [db_rows, db_cols] = db.dimensions();
    auto db_span = db.span();
        ...
        for (std::size_t seed_idx = 0; seed_idx < num_seeds; ++seed_idx) {
            ...
            for (std::size_t j_mod = 0; j_mod < num_moduli; ++j_mod) {
                for (std::uint64_t z = 0; z < stop; ++z) {
                    ...
                    std::uint64_t val = db_span[row * db_cols + col_idx];
    ...
    std::uint64_t sqrt_N) -> siper_matrix<std::uint64_t>
{
    ...
    siper_matrix<std::uint64_t> qu{sqrt_N, log_q};
...
inline auto recover_hybrid(
    RLWEContext& ctx,
    const siper_matrix<std::uint64_t>& ans,
    const siper_matrix<std::uint64_t>& hint,
    const siper_matrix<std::uint64_t>& lwe_secret,
    std::uint32_t log_q,
    std::uint64_t i_row,
    std::uint64_t n_rows) -> siper_matrix<std::uint64_t>
{
